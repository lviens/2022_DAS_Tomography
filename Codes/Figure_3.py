...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan 25 15:15:37 2022
...
#%% Directories
dir_in = '../Data'  # input directory with the correlation function files
dir_out = '../Figures'# 'DCs_no1bit'   # to save output mat files


station = 4000 # virtual source station 
station_selection = 400 # change this to select more or less stations (max is 300)
...
for method in [0,1]:
    if method == 0:
        file = dir_in  + '/Figure_2_Virt_'+str(station).zfill(4)+'.hdf5' # Read CCFs computed with 1-bit normalisation
    else:
        file = dir_in  + '/Figure_3_Virt_'+str(station).zfill(4)+'_no_1bit.hdf5'   # Read CCFs computed without 1-bit normalisation
    f = h5py.File(file, 'r')
    das = np.array(f['DAS'][:])
    
    dasf = []
    for df in range(len(das)):
        dasf.append(butter_bandpass_filter(das[df], lowcut=f1, highcut=f2 -.01, fs = delta, order=4) ) # bandpass filter
    ...
    cl = .004
    if init ==1:
        ax0 = plt.subplot(2,2,init)
    else:
        ax1 = plt.subplot(2,2,init)
    ...
    if init ==1:
        ...
        plt.title('CCFs with 1-bit',fontsize = fnt+1) 
    else:
        plt.title('CCFs without 1-bit',fontsize = fnt+1) 
    ...
    if init ==1:
        ...
        plt.text(-1, station - 15, '(a)', fontsize = fnt+2)
    else:
        plt.text(-.5, station - 15, '(b)', fontsize = fnt+2)
        ax1.tick_params(bottom=True, top=True, left=True, right=True, labelleft=False)
        ...
        rect = plt.Rectangle( (11, 4010) ,  3.5, 130, alpha = .75, facecolor = 'w', edgecolor = 'k',  zorder = 1 )
        ax1.add_patch(rect)
        norm= matplotlib.colors.Normalize(vmin=-1.001, vmax=1.001)

        sm2 = plt.cm.ScalarMappable(norm=norm )
        sm2.set_array([])
        colorbar_ax = fig.add_axes([0.82, 0.83, 0.018, 0.1])
        cb2 =fig.colorbar(sm2, cax=colorbar_ax, ticks=np.arange(-1.0,1.01,1) )
        cb2.ax.set_ylabel('Norm. ampl.', fontsize = fnt)
        plt.yticks(fontsize = fnt)
        
        
    #%% Select data 
    staini = station  
    stafin =  station_selection
    data1 = (dasf[:stafin, int(dasf.shape[1]/2):])
    data = data1[:,:int(80*delta)]
    epdist = np.arange(0, (stafin)*spatial_samp, spatial_samp) 
 ...
 #%% Plot data
    ...
    if init ==1:
        ax2=fig.add_subplot(2,2,init+2)
        plt.text(-.1, 5.2, '(c)', fontsize = fnt+2)
        plt.ylabel('Phase velocity (km/s)', fontsize = fnt)
        plt.title('Dispersion image from CCFs with 1-bit', fontsize = fnt +1 )  
    else:
        ax3=fig.add_subplot(2,2,init+2)
        plt.text(-.05, 5.2, '(d)', fontsize = fnt+2)
        plt.title('Dispersion image from CCFs without 1-bit', fontsize = fnt+1 ) 
    ...
    if init==2:
        for n in np.arange(0,10,1):
            dx = spatial_samp
            f = np.arange(.25,5,.01)
            vfin = f/((1/(2*dx*station_selection))+n/dx/station_selection)
            plt.plot(f,vfin,'w', linewidth = 1)  
    ...
    if init ==1:
        ...
        ax3.tick_params(bottom=True, top=True, left=True, right=True, labelleft=False)
        ...
        sm2.set_array([])
        colorbar_ax = fig.add_axes([0.923, 0.15, 0.018, 0.2])
        cb2 =fig.colorbar(sm2, cax=colorbar_ax, ticks=np.arange(0,1.01,.2) )
        cb2.ax.set_ylabel('Norm. amplitude\n  ', fontsize = fnt)
        plt.yticks(fontsize = fnt)

        
    for i in np.arange(len(rayf2)):
        if init ==1:
            ax2.scatter(rayf2[i],rayDC2[i] , s = 1, color = 'k' )
        else:
            ax3.scatter(rayf2[i],rayDC2[i] , s = 1, color = 'k' )
...
ax0.set_position([0.07, .56, .4, .39])
ax2.set_position([0.07, .06, .4, .39])
ax1.set_position([0.512, .56, .4, .39])
ax3.set_position([0.512, .06, .4, .39])

           
plt.show()
fig.savefig(dir_out + '/Figure_3.png', dpi=300)
